...
<div class="back">
        <table   border="1" class="query">
            <tr valign="top">
                <th valign="top">宠物名称</th>
                <th valign="top">所属人</th>
                <th valign="top">所属人唯一id</th>
            </tr>
            <%
                ArrayList arrayList = (ArrayList) session.getAttribute("pets");
                for (int i = 0; i < arrayList.size(); i++){
                    PetsTable petsTable = (PetsTable) arrayList.get(i);
            %>
            <tr>
                <form action="petsQueryPetsServlet" method="get" id="pets_<%=petsTable.getPet_name()%>">
                    <input type="hidden" name="pet_id" value="<%=petsTable.getPet_id()%>">
                </form>
                <td>
                        <a href="javascript: void(0);" onclick="form('pets_<%=petsTable.getPet_name()%>')"><%=petsTable.getPet_name()%></a>
                </td>
    ...
    <table border="0" style="margin: auto">
        <tr>
            <td>
                <form action="petsForward" method="get">
